...
class NightlyTests(unittest.TestCase):
    def test_exact_source_guard_rejects_moved_master_and_invalid_inputs(self):
        step = next(s for s in BUILD["jobs"]["base-image"]["steps"]
    ...
    def test_records_both_registries_from_source_specific_tags(self):
        steps = BUILD["jobs"]["wildcat-aux-images"]["steps"]
    ...
    def test_prs_cannot_publish_and_dispatch_cannot_skip_matrix(self):
        ...
        self.assertEqual({s["name"] for s in BUILD["jobs"]["wildcat-aux-images"]["strategy"]["matrix"]["service"]}, {'bcr-wdc-relay', 'bcr-wdc-eic-service', 'bcr-wdc-ebill-service', 'bcr-wdc-ens-service', 'bcr-wdc-demo-faucet'})
        save = next(s for s in BUILD["jobs"]["wildcat-aux-images"]["steps"] if s.get("name") == "Save the image result")
