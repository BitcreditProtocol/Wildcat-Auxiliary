...
impl BlobDescriptor {
    pub fn new(base_url: Url, hash: Sha256Hash, size: usize) -> Result<Self, anyhow::Error> {
        Ok(Self {
            ...
            uploaded: time::OffsetDateTime::now_utc().unix_timestamp(),
        })
    }
}
...
pub async fn handle_upload(State(state): State<AppState>, body: Bytes) -> impl IntoResponse {
    ...
    if let Err(e) = bitcoin::secp256k1::PublicKey::from_slice(pubkey_bytes) {
...
mod tests {
    ...
    fn create_test_state() -> AppState {
        ...
        AppState {
            relay: nostr_sdk::local_relay::LocalRelay::builder().build(),
            cfg: crate::AppConfig {
